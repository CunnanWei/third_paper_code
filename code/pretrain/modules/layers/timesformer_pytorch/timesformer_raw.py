...
def attn(q, k, v, mask=None, return_weights=False):
...
class Attention(nn.Module):
    def __init__(self, dim, dim_head=64, heads=8, dropout=0.0):
        ...
        # 是否在前向传播时记录注意力权重（默认不记录，按需开启）
        self.return_attn = False
        self.last_cls_attn = None  # 形状: (batch, heads, 1, tokens)
    ...
    def forward(
        ...
        **einops_dims,
        ...
        if self.return_attn:
            cls_out, cls_attn_weights = attn(
                cls_q, k, v, mask=cls_mask, return_weights=True
            )
            # 还原 batch 维度并缓存
            bh, q_len, tokens = (
                cls_attn_weights.shape[0],
                cls_attn_weights.shape[1],
                cls_attn_weights.shape[2],
            )
            B = bh // h
            self.last_cls_attn = (
                cls_attn_weights.view(B, h, q_len, tokens).detach().cpu()
            )
        else:
        ...
        # attention
        if self.return_attn:
            out, _ = attn(q_, k_, v_, mask=mask, return_weights=False)
        else:
